...
class _NewsScreenState extends State<NewsScreen> {
  ...
  Future<void> fetchNews(String state) async {
    if (isLoading ||
        (centralNews.isNotEmpty &&
            stateNews.isNotEmpty &&
            generalStateNews.isNotEmpty))
      return;
    ...
    final centralNewsFuture = Future.wait([
      geminiNewsService.fetchGeminiNews(
        "Summarize the latest central government agriculture schemes explicitly from the Ministry of Agriculture website: https://agriwelfare.gov.in/",
      ),
      serperService.fetchSerperNews(
        "Latest central government agriculture schemes site:agriwelfare.gov.in",
      ),
    ]);

    final stateNewsFuture = Future.wait([
      geminiNewsService.fetchGeminiNews(
        "Summarize the latest $state state government agriculture schemes explicitly from the Ministry of Agriculture website: https://agriwelfare.gov.in",
      ),
      serperService.fetchSerperNews(
        "Latest $state state agriculture schemes site:agriwelfare.gov.in",
      ),
    ]);
    ...
    final generalStateNewsFuture = Future.wait([
      ...
      serperService.fetchSerperNews("Latest $state news"),
    ]);

    final results = await Future.wait([
      centralNewsFuture,
      stateNewsFuture,
      generalStateNewsFuture,
    ]);
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 24),
                          _buildNewsSection(
                            "General State News from ${stateController.text}",
                            generalStateNews,
                          ),
                        ],
                      ),
